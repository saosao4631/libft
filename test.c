...
#define TEST(name, result, expected) printf("%s: %s (result=%d, expected=%d)\n",
	name, result == expected ? "✓" : "✗", result, expected)

#define TEST_STR(name, result, expected) printf("%s: %s (result='%s',
	expected='%s')\n", name, (result && expected && strcmp(result,
			expected) == 0) || (result == expected) ? "✓" : "✗",
	result ? result : "(nil)", expected ? expected : "(nil)")

#define TEST_PTR(name, result, expected) printf("%s: %s (result=%p,
	expected=%p)\n", name, result == expected ? "✓" : "✗", result, expected)
...
int	main(void)
{
	...
	memset(buf, 0, 10);
	ft_memset(buf, 'A', 3);
	printf("ft_memset(buf, 'A', 3): buf[0]='%c', buf[1]='%c', buf[2]='%c',
	...
	strcpy(buf, "Hello");
	ft_bzero(buf, 3);
	printf("After ft_bzero(buf, 3): buf[0]=%d buf[1]=%d buf[2]=%d (expected 0,
		0, 0)\n", buf[0], buf[1], buf[2]);
	printf("buf[3]='%c' (expected l)\n", buf[3]);
	memset(buf, 'X', 10);
	ft_bzero(buf + 2, 3);
	printf("After ft_bzero(buf+2, 3): buf[1]='%c', buf[2]=%d, buf[3]=%d,
	...
	printf("ft_strlcpy returned %d (expected 5),
		buf = \"%s\" (expected \"Hello\")\n", i, buf);
	i = ft_strlcpy(buf, "Hello", 3);
	printf("ft_strlcpy(size=3) returned %d (expected 5),
		buf = \"%s\" (expected \"He\")\n", i, buf);
	i = ft_strlcpy(buf, "Hello", 1);
	printf("ft_strlcpy(size=1) returned %d (expected 5), buf[0] =
		%d (expected 0)\n", i, buf[0]);
	...
	printf("ft_strlcat returned %d (expected 11),
		buf = \"%s\" (expected \"Hello World\")\n", i, buf);
	strcpy(buf, "Hello");
	i = ft_strlcat(buf, " World", 8);
	printf("ft_strlcat(size=8) returned %d (expected 11),
		buf = \"%s\" (truncated)\n", i, buf);
	strcpy(buf, "Hello");
	i = ft_strlcat(buf, "X", 6);
	printf("ft_strlcat(size=6) with \"X\" returned %d (expected 6),
		buf = \"%s\" (expected \"HelloX\")\n", i, buf);
	printf("\n=== ft_strnstr: find substring ===\n");
	printf("ft_strnstr(\"libft library\", \"lib\",
		20) = \"%s\" (expected \"libft library\")\n",
	...
	printf("ft_strnstr(\"libft library\", \"library\",
		20) = \"%s\" (expected \"library\")\n", ft_strnstr("libft library",
			"library", 20));
	printf("ft_strnstr(\"libft library\", \"library\", 6) =
	...
	printf("ft_strjoin(\"Hello\",
		\" World\") = \"%s\" (expected \"Hello World\")\n", str);
	free(str);
	str = ft_strjoin("", "test");
	...
	printf("ft_strtrim(\"xxxxx\", \"x\") = \"%s\" (all trimmed,
		expected empty)\n", str);
	free(str);
	...
	printf("ft_strmapi(\"abc\", add_index) = \"%s\" (expected \"ace\" a+0=a,
		b+1=c, c+2=e)\n", str);
	free(str);
	...
	printf("ft_memcmp(\"ABD\", \"ABC\", 3) = %d (expected > 0)\n",
		ft_memcmp("ABD", "ABC", 3));
	printf("ft_memcmp(\"ABC\", \"ABD\", 2) = %d (expected 0,
		compare first 2)\n", ft_memcmp("ABC", "ABD", 2));
